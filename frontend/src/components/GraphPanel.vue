...
const renderGraph = () => {
  ...
  const simulation = d3.forceSimulation(nodes)
    ...
    // Add gravity toward center to cluster isolated node groups in the center area
  ...
  const getLinkPath = (d) => {
    ...
    // Offset perpendicular to connection direction, calculated proportionally to distance
    // More edges means larger offset-to-distance ratio
    const pairTotal = d.pairTotal || 1
    const offsetRatio = 0.25 + pairTotal * 0.05 // Base 25%, increases by 5% per additional edge
