...
const parseInsightForge = (text) => {
  ...
  try {
    ...
    // Extract sub-queries - extract all, no limit
...
const parsePanorama = (text) => {
  ...
  try {
    ...
    // Extract current active facts - extract all, no limit
    ...
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        if (name) {
          ...
          // Continuation line of reason (exclude concluding summary paragraphs)
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      if (qMatch) {
        ...
        if (questions.length > 0) {
          // Special handling if first question starts with "1."
    ...
    return () => h('div', { class: 'panorama-display' }, [
      ...
      h('div', { class: 'panorama-content' }, [
        ...
        activeTab.value === 'entities' && h('div', { class: 'entities-panel' }, [
          ...
          ) : h('div', { class: 'empty-state' }, 'No entities available'),
...
const renderMarkdown = (content) => {
  ...
  // Remove leading H2 heading (## xxx), since section title is already displayed in the outer layer
  ...
  // Handle line breaks - empty lines become paragraph separators, single newlines become <br>
  html = html.replace(/\n\n/g, '</p><p class="md-p">')
  html = html.replace(/\n/g, '<br>')
  ...
  // Clean <br> before paragraph opening tags that follow block-level elements
...
const extractFinalContent = (response) => {
  ...
  // Try to find content after Chinese "最终答案:" (final answer)
  ...
  // If content is long and contains markdown format, try removing the thinking process before returning
